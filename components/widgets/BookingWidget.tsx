            ...
            <div className="flex-1 overflow-y-auto px-6 pb-32 pt-2">
                                ...
                                return (
                                    <div key={step} onClick={() => state.navigateTo(step as Step)} className="p-6 flex items-center justify-between border-2 border-neutral-100 rounded-[24px] cursor-pointer hover:border-black transition-all bg-white group shadow-sm">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-neutral-50 rounded-2xl group-hover:bg-neutral-100 transition-colors text-neutral-900"><Icon className="h-6 w-6 opacity-40" /></div>
                ...
                {state.currentView === 'profile' && (
                    <div className="space-y-6 pt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div className="space-y-5">
                            <Input className="h-14 rounded-2xl border-2 shadow-sm" value={clientData.name} onChange={e => setClientData({...clientData, name: e.target.value})} placeholder="Имя" />
                            <Input className="h-14 rounded-2xl border-2 shadow-sm" value={clientData.phone} onChange={e => setClientData({...clientData, phone: e.target.value})} placeholder="+375..." />
                            <Textarea className="rounded-2xl border-2 min-h-[100px] shadow-sm" value={clientData.comment} onChange={e => setClientData({...clientData, comment: e.target.value})} placeholder="Комментарий" />
                        </div>
                    </div>
                )}
                ...
                {state.currentView === 'success' && (
                    <div className="flex flex-col items-center justify-center h-full text-center space-y-6 pt-10">
                        <div className="h-24 w-24 bg-emerald-50 rounded-[40px] flex items-center justify-center animate-in zoom-in shadow-inner"><CheckCircle2 className="h-12 w-12 text-emerald-500" /></div>
                        <h2 className="text-3xl font-black text-neutral-900 tracking-tight">Запись создана!</h2>
                        <Button onClick={() => { state.reset(); onClose?.(); }} className="w-full h-16 bg-neutral-900 text-white rounded-[24px] font-bold shadow-xl">На главную</Button>
                    </div>
                )}
            </div>
